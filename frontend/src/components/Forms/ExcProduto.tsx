  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Erro ao excluir produto. Verifique se o ID está correto.');
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-center">
        <h1 className="text-blue-600 text-3xl font-bold mb-4">Excluir Produto</h1>
      </div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <input type="number" value={id} onChange={(e) => setId(e.target.value)} placeholder="ID do produto" className="border p-2 rounded text-black placeholder:text-gray-500" required/>
